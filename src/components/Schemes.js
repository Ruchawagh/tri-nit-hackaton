...
function Schemes() {
    ...
    const handleSubmit = async (title,desc,tags,author)=>{
        const resp = await fetch(`${baseUrl}/api/Schemes/addblog`,{
            method:'POST',
            headers:{
                "Content-type": "application/json; charset=UTF-8",
        ...
        setSchemestate(Schemestate.concat(newBlog))
        // notes.push(note);
        // setnotes(notes)
      }
    ...
    return (
        <>
            ...
            <div className="container">
                <h1 className='bg-light m-5 p-2 text-center rounded'>Schemes</h1>
